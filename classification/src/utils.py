"""
Shared utility functions for the classification module.
...
def load_config(
    path: str = "classification/config/classification_config.yaml",
) -> Dict[str, Any]:
...
def ensure_output_dirs(cfg: Dict[str, Any]) -> None:
    ...
    for key in ("figures_dir", "tables_dir", "models_dir", "reports_dir"):
...
def save_figure(fig: plt.Figure, name: str, cfg: Dict[str, Any]) -> Path:
    ...
    Args:
        ...
        name: Filename without extension (e.g. "model_comparison").
...
def get_feature_columns(df: pd.DataFrame, cfg: Dict[str, Any]) -> list[str]:
    """Return all numeric feature columns, excluding identifiers and targets."""
    exclude = set(
        cfg["columns"]["identifiers"]
        + [cfg["columns"]["target"], cfg["columns"]["target_code"]]
    )
    return [
