"""
Shared utility functions for the EDA module.

Provides configuration loading, output directory management,
figure/table saving, and plot style setup.
"""
...
def load_eda_config(path: str = "eda/config/eda_config.yaml") -> Dict[str, Any]:
    """Load EDA configuration from a YAML file."""
...
def save_figure(fig: plt.Figure, name: str, cfg: Dict[str, Any]) -> Path:
    ...
    Args:
        ...
        name: Filename without extension (e.g. "missing_heatmap").
...
def save_table(df: pd.DataFrame, name: str, cfg: Dict[str, Any]) -> Path:
    ...
    Args:
        ...
        name: Filename without extension (e.g. "basic_statistics").
        cfg: EDA configuration dict.
...
def get_numeric_columns(df: pd.DataFrame, cfg: Dict[str, Any]) -> list[str]:
    """
    Return all numeric feature columns, excluding identifiers and meta-only columns.
    Only returns columns that actually exist in the DataFrame.
    """
    exclude = set(cfg["columns"]["identifiers"] + cfg["columns"].get("meta_only", []))
    return [
...
def get_feature_group(cfg: Dict[str, Any], group: str) -> list[str]:
    """Return column list for a named feature group from config."""
    return cfg["feature_groups"].get(group, [])


def get_existing_columns(df: pd.DataFrame, columns: list[str]) -> list[str]:
    """Filter a column list to only those present in the DataFrame."""
...
def setup_plot_style(cfg: Dict[str, Any]) -> None:
    ...
    plt.rcParams.update({
        ...
        "figure.dpi": 100,  # screen display; save uses plot.dpi
